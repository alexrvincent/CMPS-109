...
void window::display() 
{
   ...
   {
      //Special Case: Object is below window
      if (object.center.ypos < 0)
      {
         object.center.ypos = window::height;
      }
      //Special Case: Object is to left of window   
      if (object.center.xpos < 0)
      {
         object.center.xpos = window::width;
      }
      //Special Case: Object is to right of window
      if (object.center.xpos > window::width)
      {
         object.center.xpos = 0;
      }
      //Special Case: Object is above window
      if (object.center.ypos > window::height)
      {
         object.center.ypos = 0;
      }
      //Otherwise draw the shape inside the window
      object.draw();
   }
   
   //Handle border drawing
   //Special case: Negative out of bounds object index
   if(window::selected_obj < window::objects.size()
                            -window::objects.size())
   {
      ++window::selected_obj;
   }
   //Special Case: Positive out of bounds index
...
void window::keyboard (GLubyte key, int x, int y) 
{
   ...
   //Depending on the key, perform an action:
   switch (key) {
